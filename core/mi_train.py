...
def mi_train_net(cfg):
    ...
    if 'MI_WEIGHTS' in cfg.CONST and cfg.TRAIN.RESUME_TRAIN:
        ...
        mi_checkpoint = torch.load(cfg.CONST.MI_WEIGHTS)
        # pdb.set_trace()
        mi_encoder.load_state_dict(mi_checkpoint['encoder_state_dict'])
        mi_trans_decoder.load_state_dict(mi_checkpoint['trans_decoder_state_dict'])
        mi_fc.load_state_dict(mi_checkpoint['fc_state_dict'])
        logging.info('Recover MI complete.')
    ...
    for epoch_idx in range(init_epoch, cfg.TRAIN.NUM_EPOCHS):
        ...
        for batch_idx, (taxonomy_names, sample_names, rendering_images,
            ...
            encoder_loss = encoder_loss + mi_encoder_loss
            refiner_loss = refiner_loss + mi_refiner_loss
        ...
        if (epoch_idx + 1) % cfg.TRAIN.SAVE_FREQ == 0 or iou > best_iou:
            file_name = 'mitrain_checkpoint-epoch-%03d.pth' % (epoch_idx + 1)
            if iou > best_iou:
                best_iou = iou
                best_epoch = epoch_idx
                file_name = 'mitrain_checkpoint-best.pth'
