...
def Rotate_Weight_1(volume, azim, elev, dist, weight_file_path):
...
def Rotate_Weight(params):
    return Rotate_Weight_1(params[0], params[1], params[2], params[3], params[4])
...
files = []
dataset_tax = [dataset_taxonomy[0], dataset_taxonomy[3], dataset_taxonomy[4], dataset_taxonomy[10]]
for taxonomy in dataset_tax:
    ...
    for sample_idx, sample_name in enumerate(samples):
        ...
        params = []
        directory = os.path.dirname(weight_file_path)
        ...
        for image_idx in rendering_image_indexes:
            ...
            params.append((volume, angels[0], angels[1], angels[2], weight_file_path))
        pool = Pool(total_views)
        pool.map(Rotate_Weight, params)
        pool.close()
        pool.join()
